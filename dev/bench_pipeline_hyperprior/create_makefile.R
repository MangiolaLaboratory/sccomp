...
root_directory = "/stornext/Bioinf/data/bioinf-data/Papenfuss_lab_projects/people/mangiola.s/PostDoc/sccomp_dev/dev/"
code_directory = glue("/stornext/Bioinf/data/bioinf-data/Papenfuss_lab_projects/people/mangiola.s/PostDoc/sccomp/dev/bench_pipeline_hyperprior/")
results_directory = glue("{root_directory}benchmark_results_hyperprior/")

tab="\t"

commands_df =
  expand_grid(
  slope = seq(1.5, 3, length.out = 10),
  n_samples = c(2, 4, 6),
  n_cell_type = c(5, 10, 20),
  max_cell_counts_per_sample = c(500),
  add_outliers = 0, # c(0, 1),
  add_hyperprior = c(0, 1, 2, 3)
) %>%

  # Add algorithm
  expand_grid(method = c("sccomp")) %>%
  ...
  mutate(output_file = sprintf("output_%s_%s_%s.rds", file_prefix, method, add_hyperprior)) %>%
  ...
  mutate(estimate_command = glue("{results_directory}{output_file}:{results_directory}{input_file}\n{tab}Rscript {code_directory}estimate_{method}.R {results_directory}{input_file} {results_directory}{output_file} {add_outliers} {add_hyperprior}"))
...
"CATEGORY=create_input\nMEMORY=40024\nCORES=2\nWALL_TIME=3600" %>%
  ...
  c(
    commands_df %>% filter(method %in% c("sccomp")) %>%  distinct(estimate_command) %>% pull(estimate_command)
  ) %>%
  ...
  c(
      ...
      pivot_wider(names_from = c(method, add_hyperprior), values_from = c(input_file, output_file, estimate_command)) %>%
