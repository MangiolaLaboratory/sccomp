...
& \alpha_{m} = \pi_{m} * \sigma_{m}; \beta_{m} = (1-\pi_{m}) * \sigma_{m} \textrm{ for } 0 > \pi > 1 \textrm{ and } \sigma > 0 & \textrm{ (2) } \\
...
& k_{m,s} \sim \mathit{BetaBinomial^{*}} \left(n_{s}, \pi_{m}, \sigma_{m} \right ) \textrm{ for } \sum_{m=1}^{M} \pi_{m} = 1 \textrm{ and } \sigma > 0 & \textrm{ (5) } \\

& \pi_{m} \sim \mathit{Dirichlet} \left(1_{M} \right ) & \textrm{ (6) } \\
...
& \pi = \mathit{softmax} \left(\mu \right) =  \frac{exp(\mu)}{\sum_{m=1}^{M} exp(\mu_{m}) } \textrm{ for } \sum_{m=1}^{M} \mu_{m} = 0  & \textrm{ (8) } \\
