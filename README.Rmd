...
data_for_plot = 
  res |> 
   pivot_wider(names_from = parameter, values_from = c(c_FDR )  )|>
 ...
 ggplot() +
  geom_boxplot(
    aes(type, proportion, fill=c_pH0<0.025),
    outlier.shape = NA, 
...
data_proportion =
      res |>
     pivot_wider(names_from = parameter, values_from = contains("c_") )  |>

      unnest(count_data) |>
      select(cell_group, sample, outlier, count, type, c_pH0_typecancer) |>
...
ggplot() +
  ...
  geom_jitter(aes(type, generated_proportions), color="blue" ,alpha=0.2, size = 0.6, data = simulated_proportion) + 
    ...
    geom_boxplot(
    aes(type, proportion, fill=c_pH0_typecancer<0.025),
    outlier.shape = NA, 
