...
functions{
  ...
  array[] int rep_each(array[] int x, int K) {
    ...
    real abundance_variability_regression(
      ...
      // Random effects (up to 5 uniform slots)
...
data{
  ...
  // ----------------------------------------------------------------------
  // Random effect blocks: up to 5 uniform "slots", one block per slot.
  ...
  //   slot 1 -> *_1 ... slot 5 -> *_5
  //
  // To go past 5 slots, paste another "slot 5" block in this file and bump
  // the array length below. There is no other architectural limit.
  // ----------------------------------------------------------------------
...
parameters{
  ...
  // ----------------------------------------------------------------------
  // Random effect parameters - 5 uniform slots.
  //
  // For each slot k:
  ...
  //
  // Hyperprior scalars sigma_mu / sigma_sigma are shared in length-5 arrays.
  // ----------------------------------------------------------------------
...
model{
  ...
  if(use_data == 1){
   ...
   target += reduce_sum(
      ...
      // Random effects (5 uniform slots)
      ncol_X_random_eff,
      X_random_effect_1,
      X_random_effect_2,
      X_random_effect_3,
